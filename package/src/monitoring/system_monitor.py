...
class SystemMonitor:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if len(values) >= 4:
                    try:
                        ...
                        metrics['memory_percent'] = (mem_used_mb / mem_total_mb) * 100 if mem_total_mb > 0 else 0.0
    ...
    def update_metrics(self):
        ...
        # GPU metrics - always try to get them
        gpu_metrics = self._get_gpu_metrics()
        if gpu_metrics:
            # Update all fields - use 0.0 as default if key missing
            self.metrics['gpu_utilization'] = float(gpu_metrics.get('utilization', 0.0))
            self.metrics['gpu_temp'] = float(gpu_metrics.get('temperature', 0.0)) if 'temperature' in gpu_metrics else None
            self.metrics['gpu_memory_percent'] = float(gpu_metrics.get('memory_percent', 0.0))
            self.metrics['gpu_power'] = float(gpu_metrics.get('power', 0.0)) if 'power' in gpu_metrics else None
        else:
            # No GPU metrics available - set to 0 to show GPU is present but idle/unavailable
