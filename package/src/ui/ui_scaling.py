...
class UIScaling:
    ...
    MIN_SCALE = 0.7  # Prevent text from becoming too small
    MAX_SCALE = 2.0
    ...
    def get_scale_factor(window: Optional[QWidget] = None, width: Optional[int] = None, height: Optional[int] = None) -> float:
        ...
        # Calculate scale based on minimum of width and height scaling
        # This ensures content fits in the smaller dimension
        width_scale = width / UIScaling.BASE_WIDTH
        height_scale = height / UIScaling.BASE_HEIGHT
        scale = min(width_scale, height_scale)  # Use minimum to ensure fit
