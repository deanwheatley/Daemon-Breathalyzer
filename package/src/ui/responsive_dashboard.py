...
class ResponsiveDashboard(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Metrics grid (no scroll, auto-scale)
        metrics_widget = QWidget()
        self.scroll_layout = QGridLayout(metrics_widget)
        self.scroll_layout.setSpacing(15)
        self.scroll_layout.setContentsMargins(0, 0, 0, 0)
        
        layout.addWidget(metrics_widget, 1)  # Give it stretch factor
    ...
    def _create_metric_widgets(self):
        ...
        # Fan speeds
        self.metric_widgets['cpu_fan'] = AnimatedMetricCard("CPU Fan", "RPM", GAME_COLORS['accent_cyan'])
        self.metric_widgets['gpu_fan'] = AnimatedMetricCard("GPU Fan", "RPM", GAME_COLORS['accent_orange'])
    ...
    def _update_layout(self):
        ...
        # Widget order
        widget_order = [
            'cpu_percent', 'cpu_temp', 'cpu_freq', 'cpu_fan',
            'memory', 'memory_used', 'gpu_usage', 'gpu_temp',
            'gpu_memory', 'gpu_fan', 'fps',
    ...
    def update_metrics(self, metrics: Dict, history: Dict):
        ...
        if 'gpu_temp' in self.metric_widgets:
            ...
            if gpu_temp is not None and gpu_temp > 0:
                self.metric_widgets['gpu_temp'].set_value(gpu_temp)
            else:
                self.metric_widgets['gpu_temp'].set_value(None, text="N/A")
        if 'gpu_memory' in self.metric_widgets:
            gpu_mem = metrics.get('gpu_memory_percent')
            if gpu_mem is not None and gpu_mem >= 0:
                self.metric_widgets['gpu_memory'].set_value(gpu_mem)
            else:
                self.metric_widgets['gpu_memory'].set_value(None, text="N/A")
            ...
            if avg_window > 0:
                ...
                if avg_value is not None:
                    ...
                    if gpu_util is not None and gpu_util >= 0:
                        ...
                        self.metric_widgets['gpu_usage'].set_value(None, text="N/A")
            else:
                gpu_util = metrics.get('gpu_utilization')
                if gpu_util is not None and gpu_util >= 0:
                    self.metric_widgets['gpu_usage'].set_value(gpu_util)
        ...
        if 'cpu_fan' in self.metric_widgets:
            self.metric_widgets['cpu_fan'].set_value(cpu_fan_rpm, decimals=0) if cpu_fan_rpm else \
                self.metric_widgets['cpu_fan'].set_value(None, text="N/A")
        if 'gpu_fan' in self.metric_widgets:
            self.metric_widgets['gpu_fan'].set_value(gpu_fan_rpm, decimals=0) if gpu_fan_rpm else \
                self.metric_widgets['gpu_fan'].set_value(None, text="N/A")
