...
class HistoryTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.stats_label.setStyleSheet(f"color: {GAME_COLORS['text_primary']}; background-color: transparent;")
    ...
    def update_table(self):
        ...
        if valid_values:
            ...
            stats_text = (
                f"<span style='color: {GAME_COLORS['accent_blue']};'><b>Statistics for {metric_name}:</b></span><br>"
                f"<span style='color: {GAME_COLORS['text_primary']};'>Average: {avg:.2f} | Min: {min_val:.2f} | Max: {max_val:.2f}<br>"
                f"Data points: {len(valid_values)}</span>"
            )
