...
class FanCurveBuilderWidget(QWidget):
    ...
    def _load_curve(self):
        ...
        if filename:
            loaded_data = self.file_manager.load_curve(Path(filename).stem)
            if loaded_data:
                # Convert dict to CurveData object
                curve_data = CurveData(loaded_data.get('name', 'Unnamed'), loaded_data.get('description', ''))
                curve_data.points = loaded_data.get('points', [(30, 0), (90, 0)])
                curve_data.created = loaded_data.get('created', datetime.now().isoformat())
                curve_data.modified = loaded_data.get('modified', datetime.now().isoformat())
    ...
    def _save_curve(self):
        ...
        if self.current_file:
            curve_dict = {
                'name': self.canvas.curve_data.name,
                'description': self.canvas.curve_data.description,
                'points': self.canvas.curve_data.points,
                'created': self.canvas.curve_data.created,
                'modified': datetime.now().isoformat()
            }
            if self.file_manager.save_curve(self.current_file, curve_dict):
    ...
    def _save_as_curve(self):
        ...
        if filename:
            ...
            if self.file_manager.save_curve(Path(filename).stem, curve_dict):
                self.current_file = Path(filename).stem
                QMessageBox.information(self, "Success", "Curve saved successfully!")
            else:
                QMessageBox.warning(self, "Error", "Failed to save curve.")
