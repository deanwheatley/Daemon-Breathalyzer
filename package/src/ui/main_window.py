...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Set initial size and enforce minimum size
        self.setGeometry(100, 100, 1200, 800)
        self.setMinimumSize(1000, 700)  # Minimum size to ensure UI remains readable
        self._current_scale = 1.0
        self._aspect_ratio = 1.5  # 3:2 aspect ratio (width:height)
    ...
    def resizeEvent(self, event):
        """Handle window resize to update scaling and maintain aspect ratio."""
        super().resizeEvent(event)
        self._update_scaling()
        
        # Enforce aspect ratio when user resizes (optional - commented out to allow free resize)
        # Uncomment the following lines to enforce strict aspect ratio:
        # new_size = event.size()
        # width = new_size.width()
        # height = new_size.height()
        # 
        # # Calculate expected dimensions based on aspect ratio
        # expected_height = int(width / self._aspect_ratio)
        # expected_width = int(height * self._aspect_ratio)
        # 
        # # Adjust to maintain aspect ratio (prefer width adjustment)
        # if abs(height - expected_height) > 10:  # Tolerance of 10px
