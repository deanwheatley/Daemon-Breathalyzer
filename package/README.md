...
- **⚙️ Multi-Brand Support**: Works with ASUS (asusctl), Dell (i8kutils), and generic laptops (PWM control)
...
2. Search for **"Daemon Breathalyzer"**
...
2. Navigate to: `~/projects/daemon-breathalyzer`
3. Double-click `daemon-breathalyzer-launcher.sh`
...
├── tests/            # Test files
├── daemon-breathalyzer-launcher.sh  # Standalone launcher script
...
**Launch Options:**
- Application Menu → Search "Daemon Breathalyzer"
- Double-click `daemon-breathalyzer-launcher.sh`
- Run `./daemon-breathalyzer-launcher.sh` from terminal
